  ...
  for row in range(len(objects)):
      for column in range(len(objects)):
          if row == column:
              ...
              h_dist, v_dist = normalized_pixel_distance(node1, node2, _screen_width, _screen_height)
...
def _build_neighbors(node, view_hierarchy_leaf_nodes, _screen_width, _screen_height):
  ...
  if view_hierarchy_leaf_nodes is None:
    return None
  vh_relation = get_view_hierarchy_leaf_relation(view_hierarchy_leaf_nodes,
                                                  _screen_width, _screen_height)
  _neighbor = _get_single_direction_neighbors(view_hierarchy_leaf_nodes.index(node), 
